...
//    }
    ...
    @PostMapping("/forgot-password")
    public ResponseEntity<?> forgotPassword(@RequestBody ForgottenPasswordDto forgottenPasswordDto) {
        try {
            UserDto userDto = userService.resetPassword(forgottenPasswordDto);

            // Връщаме username и новата парола
            Map<String, String> response = new HashMap<>();
            response.put("username", userDto.getUsername());
            response.put("newPassword", userDto.getPassword());
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(Map.of("error", "An error occurred while trying to reset your password."));
        }
    }
