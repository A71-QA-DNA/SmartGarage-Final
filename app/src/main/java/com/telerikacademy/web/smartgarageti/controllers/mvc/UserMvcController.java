...
//            return "redirect:/users/" + currentUser.getId() + "/details";
//        } catch (IOException e) {
...
//    }
    ...
    public String handleForgotPassword(
        ...
        try {
            UserDto userDto = userService.resetPassword(forgottenPasswordDto);

            redirectAttributes.addFlashAttribute("login", new LoginDto());
            redirectAttributes.addFlashAttribute("newUsername", userDto.getUsername());
            redirectAttributes.addFlashAttribute("newPassword", userDto.getPassword());
            redirectAttributes.addFlashAttribute("successMessage", "You can see the new password.");
            redirectAttributes.addFlashAttribute("source", "forgot");

            return "redirect:/auth/login";
        } catch (EntityNotFoundException e) {
            redirectAttributes.addFlashAttribute("errorMessage", "No account found with that email.");
        } catch (Exception e) {
            redirectAttributes.addFlashAttribute("errorMessage", "An error occurred while trying to reset your password.");
        }
    ...
    public String createUser(@Valid @ModelAttribute("userCreationDto") UserCreationDto userCreationDto,
        ...
        try {
            ...
            UserDto userDto = userService.createCustomerProfile(currentUser, userCreationDto);
            model.addAttribute("username", userDto.getUsername());
            model.addAttribute("password", userDto.getPassword());
            model.addAttribute("userType", "mechanic");
            model.addAttribute("message", "Mechanic user has been successfully created!");
            ...
            if (message.contains("Username")) {
                bindingResult.rejectValue("username", "error.userCreationDto", message);
            } else if (message.contains("Email")) {
                bindingResult.rejectValue("email", "error.userCreationDto", message);
            } else if (message.contains("Phone number")) {
                bindingResult.rejectValue("phoneNumber", "error.userCreationDto", message);
            }
    ...
    public String createMechanic(@Valid @ModelAttribute("userCreationDto") UserCreationDto userCreationDto,
        ...
        try {
            User currentUser = authenticationHelper.tryGetUserFromSession(session);
            UserDto createdMechanic = userService.createMechanicProfile(currentUser, userCreationDto);

            model.addAttribute("username", createdMechanic.getUsername());
            model.addAttribute("password", createdMechanic.getPassword());
            model.addAttribute("userType", "mechanic");
            model.addAttribute("message", "Mechanic user has been successfully created!");
...
//    }
    ...
    public String createEmployee(@Valid @ModelAttribute("userCreationDto") UserCreationDto userCreationDto,
        ...
        try {
            User currentUser = authenticationHelper.tryGetUserFromSession(session);
            UserDto createEmployee = userService.createEmployeeProfile(currentUser, userCreationDto);

            model.addAttribute("username", createEmployee.getUsername());
            model.addAttribute("password", createEmployee.getPassword());
            model.addAttribute("userType", "employee");
            model.addAttribute("message", "Employee user has been successfully created!");
